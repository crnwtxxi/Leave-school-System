...
var dataAcquisition = {
    store:{ //配置项
        ...
        sendUrl      : "http://106.15.206.229/acquisition/push",   //log采集地址（需配置）
        selector     : 'input',     //通过控制输入框的选择器来限定监听范围,使用document.querySelector进行选择，值参考：https://www.runoob.com/cssref/css-selectors.html
        acRange      : ['text'],   //输入框采集范围, 不建议采集密码输入框内容，此处只为展示用
        userSha      : 'userSha',   //用户标识
        // classTag     : '',          //自动埋点,数据大
        classTag     : 'isjs-ac',   //主动埋点标识, 自动埋点时请配置空字符串
        maxDays      : 5,           //cookie期限
        acbLength    : 1,           //点击元素采集层数，自动埋点时会向上层查找，该选项可以配置查找层数
        useStorage   : true,       //自动检测是否使用storage，不要手动更改
    ...
    init: function () {
        ...
        if (this.util.isNullOrEmpty(this.util.getCookie(this.store.userSha))) {
            this.util.setCookie(this.store.userSha, this.util.getUsername())
        }
    ...
    postData: function () {//数据上报
        ...
        var dataText = "uuid=" + uuid.toString();
        for (var i in data) dataText = dataText + "&acData=" + JSON.stringify(data[i]);
